<?php
//this line will cause conflict one more time
require_once(BASE_PATH.'/base_parser.php');

class VBRMoscowBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.vrbmoscow.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, ' </tr>','</table', 0, true);		
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			$symb = trim(strip_tags($data['data']));
			$symbol = preg_replace('/[\W\d]+/', '', $symb);
			...
			if($symbol == 'VND')
			{
				$qty = rtrim(preg_replace('/[^0-9 ]/', '', $symb), ' ');	//everything that is not numbers or space is replaced with nothing

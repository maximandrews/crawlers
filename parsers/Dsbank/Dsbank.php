...
class Dsbank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://dsbank.ru/dlja-chastnykh-lic/obmen-valjuty/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, 'Курс продажи','EUR/USD', 0, true);
		$cnt = trim($data['data']);
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			if ($symbol == 'Доллар США') {     //checking currency name by its heading
				$symbol = 'USD';
			} else {
				$symbol = 'EUR';
			}

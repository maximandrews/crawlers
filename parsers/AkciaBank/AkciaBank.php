...
class AkciaBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.akcia-bank.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<table cellpadding="0" cellspacing="0" class="kurs_price">','</table>', 0, true);
		$cnt = trim($data['data']);
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			//Setting symbol depending on row
			if($start == 0)
				$symbol = 'USD';
			else
				$symbol = 'EUR';

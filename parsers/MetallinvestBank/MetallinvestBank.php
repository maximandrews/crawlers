...
class MetallinvestBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://metallinvestbank.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<div class="currency">','</table>', 0, true);
		$cnt = trim($data['data']);
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			if(is_int(strpos($rcnt, 'наличных'))) {
				$dtype = 'Cash';
				continue;
			}elseif(is_int(strpos($rcnt, 'По картам'))){
				$dtype = '';
				continue;
			}

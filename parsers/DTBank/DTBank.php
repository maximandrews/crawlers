...
class DTBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.dtb1.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<table style="font-size: 10px"','</table>', 0, true);
		$cnt = iconv('windows-1251', 'UTF-8', trim($data['data']));
		$cnt = trim(findData($cnt, '</caption>','Курс ЦБ', 0, true)['data']);
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			if (strpos($rcnt, '<th colspan=2>') > 0)
				continue;

			$rcnt = preg_replace("/<th[^>]+>/is", '<td>', $rcnt);
			$rcnt = str_replace('</th>', '</td>', $rcnt);

...
class BankKiev extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.bank.kiev.ua/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<table width="95%" cellpadding="0" cellspacing="0" class="curency">', '</table>', 0, true);
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			//Skipping second HTML table cell (NBU rate)
			$rstart = $data['end'];
			$data = findData($rcnt, '<td>', '</td>', $rstart, true);

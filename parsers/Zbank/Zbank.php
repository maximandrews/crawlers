...
class Zbank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.z-bank.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<div id="currency1">','currency2', 0, true);
		$cnt = trim($data['data']);
		...
		while(is_int($start) && $row = findData($cnt, '<div>', '</div>', $start, true)) {
			...
			$rstart = 0;
			$data = findData($rcnt, '<h1>', '</h1>', $rstart, true);

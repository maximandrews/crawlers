...
class MestnyBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.mkbank.ru/valuta/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		while(is_int($start) && $row = findData($cnt, '<TR>', '</TR>', $start, true)) {
			...
			//Replacing cyrillic currency names with suiting 3-character names
			$symbol = str_replace('Доллары США', 'USD', $symbol);
			$symbol = str_replace('ЕВРО', 'EUR', $symbol);
			$symbol = str_replace('Фунт стерлингов Соединенного королевства', 'GBP', $symbol);

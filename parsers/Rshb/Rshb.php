...
class Rshb extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.rshb.ru/natural/exchange/cross.php');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<table style="margin-bottom: 20px;">','EUR/USD', 0, true);
		$cnt = iconv('windows-1251', 'UTF-8', trim($data['data']));
		
		// Replacing currency names in Russian with currency symbols
		$cnt = str_replace('Доллар США*','USD',$cnt);
		$cnt = str_replace('ЕВРО*','EUR',$cnt);
		$cnt = str_replace('Фунт стерлингов Соединенного королевства','GBP',$cnt);
		$cnt = str_replace('Швейцарский франк','CHF',$cnt);

...
class NacKorpBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://ncorpbank.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, 'id="cur_0">','</ul>', 0, true);
		$cnt = trim($data['data']);

		//Looping trough all rates rows
		$start = 0;
		while(is_int($start) && $row = findData($cnt, '<strong', '</li>', $start, true)) {
			$rcnt = $row['data'];
			$start = $row['end'];
			...
			$rstart = $data['end']+5;
			$data = findData($rcnt, 'n>', '<', $rstart, true);

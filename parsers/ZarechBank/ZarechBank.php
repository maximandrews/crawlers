...
class ZarechBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.zarech.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<td colspan="4"><a class="help"', '</table>', 0, true);
		$cnt = trim($data['data']);
		...
		$start = strpos($cnt, '</tr>');
		while(is_int($start) && $row = findData($cnt, '<tr bgcolor', '</tr>', $start, true)) {
			$rcnt = $row['data'];
			$start = $row['end'];

...
class VkaBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.vkabank.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<col span=2 align="center">','<th colspan="3">', 0, true);
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			if(preg_match("/от ([\d ]+)/i", $rcnt, $regs)) {
				$qty = preg_replace("/\D+/", '', $regs[1]);
			}elseif(preg_match("/до ([\d ]+)/i", $rcnt, $regs)) {
				$qty = 1;
			}

			//Looking for currency symbol (first HTML table cell)
			$rstart = 0;		
			$data = findData($rcnt, '<th>', '<br>', $rstart, true);

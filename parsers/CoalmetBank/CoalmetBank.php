...
class CoalmetBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.coalmetbank.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, "<table class='exchange-table'>", '</table>', 0, true);
		$cnt = trim($data['data']);
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			//Skipping second HTML table cell (CB rate)
			$rstart = $data['end'];
			$data = findData($rcnt, '<td>', '</td>', $rstart, true);

...
class MasterCapitalBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.master-capital.ru/main.php?n=9&f=1');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, 'class=bigl id=bl_light','</table>', 0, true);
		$cnt = trim($data['data']);

		//Unifying tags for simplifying search
		$cnt = preg_replace("/<td[^>]+>/is", '<td>', $cnt);
		$cnt = preg_replace("/<\/?(p)[^>]*>/is", '', $cnt);

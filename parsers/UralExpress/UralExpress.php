...
class UralExpress extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.uralexpress.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, 'exchange-curse','<div id="footer">', 0, true);
		$cnt = trim($data['data']);

		//Unifying tags
		$cnt = preg_replace("/<fieldset[^>]+>/is", '<fieldset>', $cnt);
		...
		$start = 0;
		$type = '';
		while(is_int($start) && $row = findData($cnt, '<fieldset>', '</fieldset>', $start, true)) {
			$rcnt = $row['data'];
			$start = $row['end'];

			//Looking for currency symbol (first HTML table cell)
			$rstart = 0;
			$data = findData($rcnt, '<legend>', '</legend>', $rstart, true);

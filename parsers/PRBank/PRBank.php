...
class PRBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.prb.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<td colspan="7"><img src="/x.gif" alt="" height="3" width="1"></td','</table>', 0, true);
		$cnt = trim($data['data']);
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			$rstart = stripos($rcnt, '</td>', $rstart);
			
			//Looking for buying rate (second HTML table cell)
			$data = findData($rcnt, 'width="1"></td>', '</td>', $rstart, true);

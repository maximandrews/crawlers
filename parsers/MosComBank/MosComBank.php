...
class MosComBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.moscombank.ru/razdel/razdel30.html');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<td style="padding-left:22px"','</table>', 0, true);
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			$rstart = $data['end'];
			$data = findData($rcnt, '<br>', '</td>', $rstart, true);

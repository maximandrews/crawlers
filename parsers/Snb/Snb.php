...
class Snb extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.snb.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<span class="b-abbr">','</table>', 0, true);
		$cnt = trim($data['data']);
		
		//Looping trough all rates rows
		$start = 0;
		$i = 1; // type of currency symbol
		while(is_int($start) && $row = findData($cnt, '<td width="40" align="right" nowrap="nowrap">', '</td>', $start, true)) {
			$rcnt = $row['data'];
			$start = $row['end'];
			...
			$rstart = 0;
			$data = findData($rcnt, '<p>','</p>', $rstart, true);

...
class KuzBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.kuzbank.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<td width="25">&nbsp','<td colspan="4" class="title">', 0, true);
		$cnt = trim($data['data']);

		//Looping trough all rates rows
		$start = 0;
		while(is_int($start) && $row = findData($cnt, ';', '<tr>', $start, true)) {
			$rcnt = $row['data'];
			$start = $row['end'];

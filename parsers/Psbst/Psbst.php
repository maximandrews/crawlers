...
class Psbst extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.psbst.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<table cellspacing="0" border="0" id="ctl00_wpMain_gwpcrBank_crBank_gvUnit"','</table>', 0, true);
		$cnt = trim($data['data']);
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			$rstart = stripos($rcnt, '</td>', $rstart+1);

...
class BankSevcred extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.sevcred.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			$rstart = $data['end'];
			$data = findData($rcnt, '<td>', '</td>', $rstart, true);
			$buyCash = $this->CheckRate(str_replace(' ', '', trim(strip_tags($data['data']))));

			//Looking for selling rate (third HTML table cell)
			$rstart = $data['end'];
			$data = findData($rcnt, '<td>', '</td>', $rstart, true);
			$sellCash = $this->CheckRate(str_replace(' ', '', trim(strip_tags($data['data']))));
			$sellCash = str_replace(' ', '', $sellCash);

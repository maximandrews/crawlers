...
class BankStrategia extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.strat.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<tr style="height: 15px; border-bottom: solid, 1px, #101010">', '</table>', 0, true);
		$cnt = trim($data['data']);
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			//Skipping first HTML table cell (image)
			$rstart = 0;
			$data = findData($rcnt, '<td>', '</td>', $rstart, true);

			//Looking for currency symbol (second HTML table cell)
			$rstart = $data['end'];
			$data = findData($rcnt, '<td>', '</td>', $rstart, true);
			$symbol = trim(strip_tags($data['data']));
			$symbol = str_replace('Доллар США*', 'USD', $symbol);
			$symbol = str_replace('ЕВРО*', 'EUR', $symbol);
			$symbol = str_replace('Шекель**', 'ILR', $symbol);

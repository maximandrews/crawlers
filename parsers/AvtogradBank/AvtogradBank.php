...
class AvtogradBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.avtogradbank.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			if (preg_match("/\([^\)]+\)/s", $symbol, $amount)) {
				$symbol = str_replace($amount[0], '', $symbol);
				$amount = preg_replace("/\D+/is", '', $amount[0]);
			}else
				$amount = 1;  //Setting default value for minimal deal amount

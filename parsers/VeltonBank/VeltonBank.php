...
class VeltonBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.veltonbank.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<table cellspacing="0" cellpadding="2" style="border-collapse: collapse; text-align: center;">','</table>', 0, true);
		$cnt = trim($data['data']);

		//Cutting content
		$cnt = substr($cnt, stripos($cnt, '</tr>'));

		//converting to rows
		$cnt = str_replace('<td>&nbsp;</td>', '</tr><tr>', $cnt);

...
class NovyVekBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.newbank.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, 'продажа</td></tr>','</tbody>', 0, true);		
		$cnt = trim($data['data']);
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			$rstart = $data['end']+17;
			$data = findData($rcnt, '<strong>', '<', $rstart, true);

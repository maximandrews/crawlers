...
class TulapromBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.tulaprombank.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<div class="kursvalut">','</table>', 0, true);
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			//Replacing currency names in Russian with its symbol
			if ($symbol == 'Доллар США')
				$symbol = 'USD';
			elseif ($symbol == 'Евро')
				$symbol = 'EUR';
			elseif ($symbol == 'Укр. гривна')
				$symbol = 'UAH';

...
class BankTC extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.banktc.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<table border="0" cellpadding="0" cellspacing="0" width="202">','</table>', 0, true);
		$cnt = trim($data['data']);

		//Unifying <tags>
		$cnt = preg_replace("/<tr[^>]+>/is", '<tr>', $cnt);
		$cnt = preg_replace("/<td[^>]+>/is", '<td>', $cnt);
		$cnt = preg_replace("/<font[^>]+>/is", '<font>', $cnt);

		//Inserting symbol in order to recognize currency values
		$cnt = str_replace ('/','//',$cnt);
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '<//tr>', $start, true)) {
			...
			$rstart =  0;
			$data = findData($rcnt, '<font>','<//font>', $rstart, true);
			$symbol = trim(strip_tags($data['data']));

			//Looking for buying rate (second HTML table cell)
			$rstart = $data['end'];
			$data = findData($rcnt, '<font>', '/', $rstart, true);

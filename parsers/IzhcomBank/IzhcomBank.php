...
class IzhcomBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.izhcombank.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, 'ass="first">','</table>', 0, true);		
		$cnt = trim($data['data']);

		//Looping trough all rates rows
		$start = 0;
		while(is_int($start) && $row = findData($cnt, '<td>', '</tr>', $start, true)) {
			$rcnt = $row['data'];
			$start = $row['end'];

			//Looking for currency symbol (first HTML table cell)
			$rstart = 0;
			$data = findData($rcnt, 'ed">', '<', $rstart, true);

...
class VestInterBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://westinterbank.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			//Looking for currency symbol (HTML table title cell)
			$rstart = 0;
			...
			$rstart = $data['end']+5;
			$data = findData($rcnt, '>', '<br />', $rstart, true);

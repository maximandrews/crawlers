...
class Bankhimik extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.bankhimik.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<table border=','</table>', 0, true);
		$cnt = trim($data['data']);
		...
		while(is_int($start) && $row = findData($cnt, ' <tr>', '</tr>', $start, true)) { 
			...
			$rstart = 0;
			$data = findData($rcnt, '<p align="center">', '</p>', $rstart, true);

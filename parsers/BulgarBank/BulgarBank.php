...
class BulgarBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.bulgarbank.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			$rstart = 0;
			$data = findData($rcnt, '<td class="valute">', '</td>', $rstart, true);
			$symbol = trim(strip_tags($data['data']));

			//Modifying currency symbol
			if ($symbol == 'EURO')
				$symbol = 'EUR';

...
class AgrorosBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', Array(
			'url' => 'http://www.agroros.ru/',
			'cookie' => 'locality=Saratov'
		));
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<span title','</tbody>', 0, true);
		$cnt = trim($data['data']);
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			if(is_int(strpos($rcnt, 'Курсы ЦБ РФ')))
				break;

...
class SwedbankBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.swedbank.ru/swedpack/rates/cash.js');
	}
		...
		*/
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			if (preg_match('/(\d+) /is', $symbol, $regs)) {
				$amount = $regs[1];
				$symbol = str_replace($regs[0], '', $symbol);
			}
			else
				$amount = 1;

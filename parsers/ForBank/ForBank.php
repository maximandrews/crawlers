...
class ForBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://xn--80ab2alcm0a.xn--p1ai/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<div class="quotes-hsell">','text/JavaScript">', 0, true);
		$cnt = trim($data['data']);

		//Unifying <tags>		
		$cnt = preg_replace("/<td[^>]+>/is", '<td>', $cnt);		
		$cnt = preg_replace("/<img[^>]+>/is", '<img>', $cnt);
		$cnt = str_replace('language', '</tr>',$cnt);
		$cnt = str_replace('<div class="quotes">', '</tr><tr><div class="quotes">', $cnt);

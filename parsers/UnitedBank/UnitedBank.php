...
class UnitedBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.united.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$cnt = preg_replace("/<\/?(font|p)[^>]*>/is", '', $cnt);
		$cnt = str_replace('p./', '</td><td>', $cnt);
		$cnt = str_replace('&nbsp;', '', $cnt);
		$cnt = str_replace('p.', '', $cnt);
		$cnt = str_replace('EURO', 'EUR', $cnt);
		$cnt = str_replace('1', '', $cnt);

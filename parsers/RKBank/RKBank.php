...
class RKBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.bank39.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<tbody>','Euro/USD', 0, true);
		$cnt = trim($data['data']);
		...
		$cnt = str_replace('Доллар <br>США','USD',$cnt);
		$cnt = str_replace('Евро','EUR',$cnt);
		$cnt = str_replace('Литовский <br>лит','LTL',$cnt);
		$cnt = str_replace('Польский <br>злотый','PLN',$cnt);

...
class XcityBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.xcitybank.com.ua/curs/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<div class="txt">','</table>', 0, true);
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			if(preg_match("/([A-Z]{3})/", $symbol, $regs))
				$symbol = $regs[1];

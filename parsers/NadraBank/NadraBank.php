...
class NadraBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.nadra.com.ua/site/page.php?lang=RU&id_part=286');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<td class="curvalsh"','<td colspan=8>', 0, true);
		$cnt = trim($data['data']);

		// Unifying and modifying tags for simplifying search
		$cnt = preg_replace("/<td[^>]+>/is", '<td>', $cnt);
		$cnt = preg_replace("/<\/?(img|font)[^>]*>/is", '', $cnt);

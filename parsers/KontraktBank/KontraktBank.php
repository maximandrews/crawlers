...
class KontraktBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.kontrakt.ua/uk/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<div id="currency">','"telephone">', 0, true);
		$cnt = trim($data['data']);

		// Modifying tags for simplifying search
		$cnt = preg_replace("/<div[^>]+>/is", '<div>', $cnt);
		$cnt = preg_replace("/<span[^>]+>/is", '<span>', $cnt);
		$cnt = str_replace('<span>Валюта','</tr><tr><span>Валюта',$cnt);
		$cnt = str_replace('Купівля','',$cnt);
		$cnt = str_replace('Продаж','',$cnt);
		$cnt = str_replace('Валюта','',$cnt);
		$cnt = str_replace('<span></span>','',$cnt);
		$cnt = str_replace('<div id=','</tr>',$cnt);

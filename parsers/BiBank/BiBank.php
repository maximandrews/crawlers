...
class BiBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.bibank.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		//Setting variable $amount to its primary value
		$amount = 1;
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			if (strpos($data['data'], 'у.е') > 0 && preg_match('/\d+/i', $data['data'], $res)) {
				$amount = $res[0];
				continue;
			}

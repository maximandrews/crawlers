...
class Vologdabank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.vologdabank.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<div class="valuta"','<div class="block2">', 0, true);
		$cnt = iconv('windows-1251', 'UTF-8', trim($data['data']));

		//Unifying tags
		$cnt = preg_replace("/<span>[^>]+<\/span>/is", '', $cnt);
		$cnt = preg_replace("/<tr[^>]+>/is", '<tr>', $cnt);
		$cnt = preg_replace("/<td[^>]+>/is", '<td>', $cnt);
		$cnt = trim(strip_tags($cnt, '<td><div>'));
		$cnt = str_replace('<td></td>', '', $cnt);

		//Looping trough all rates rows
		$start = 0;
		while(is_int($start) && $row = findData($cnt, 'style="background:url', '</div>', $start, true)) {
			$rcnt = $row['data'];
			$start = $row['end'];

			//Looking for currency symbol (attribute of the <div> tag)
			$rstart = 0;
			$data = findData($rcnt, "('/img/", '.gif', $rstart, true);
			$symbol = trim(strip_tags($data['data']));
			$symbol = str_replace('dollar', 'USD', $symbol);
			$symbol = str_replace('evro', 'EUR', $symbol);

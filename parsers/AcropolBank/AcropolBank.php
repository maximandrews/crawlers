...
class AcropolBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.acropol.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<br clear="all">','</table>', 0, true);
		$cnt = trim($data['data']);
		...
		$cnt = str_replace('</td><td>USD</td>', '</td>', $cnt);
		$cnt = str_replace('</td><td>EUR</td>', '</td>', $cnt);

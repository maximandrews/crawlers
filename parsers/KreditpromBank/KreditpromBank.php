...
class KreditpromBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.kreditprombank.com/ua/personal');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			//Default value of minimal deal;
			$amount = 1;

			if (preg_match('/\d+/is', $symbol, $regs)) {
				$amount = $regs[0];
			...
			$rstart = $data['end'];
			$data = findData($rcnt, '<td>', '<br/>', $rstart, true);

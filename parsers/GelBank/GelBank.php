...
class GelBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://gelbank.ru/images/rates.htm');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<div>','</div>', 0, true);
		$cnt = trim($data['data']);
		...
		$cnt = preg_replace("/\s+</is", '<', $cnt);
		$cnt = substr($cnt, stripos($cnt, '<td></td><td></td><td></td></tr>')+32);

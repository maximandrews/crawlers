...
class BinBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.binbank.ru/rus/private/valuta/special_kurs/cities/mos/index.wbp');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<p align="center" class="oranje">','<p align="center">', 0, true);
		$cnt = trim($data['data']);

		//Unifying tags
		$cnt = preg_replace("/<td[^>]+>/is", '<td>', $cnt);
		$cnt = preg_replace("/<tr[^>]+>/is", '<tr>', $cnt);
		$cnt = preg_replace("/<\/?(img|strong|table|p)[^>]*>/is", '', $cnt);
		$cnt = str_replace ('&nbsp;','',$cnt);

...
class BankAkcept extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.akcept.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, 'id="valuta"','</table>', 0, true, false);
		$cnt = trim($data['data']);
		...
		$cnt = preg_replace('/<(\w+)[^>]*>/i', '<\\1>', $cnt);

		// Remove headings
		// 2 times because, there are two table headings
		for($i = 0; $i < 2; $i++) {

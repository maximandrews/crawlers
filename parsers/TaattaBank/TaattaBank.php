...
class TaattaBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://taata.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<table>','Gold', 0, true);
		$cnt = trim($data['data']);
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			$symbol = str_replace('&#163;', 'GBP', $symbol);
			$symbol = str_replace('&#x00A5;', 'CNY', $symbol);
			if ($symbol == "CNY")
				$amount = 10;

			//Skipping second HTML table cell (image)
			$rstart = $data['end'];
			$data = findData($rcnt, '<td>', '</td>', $rstart, true);

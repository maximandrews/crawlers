...
class CapitalBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://capitalbank.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			if(preg_match("/([A-Z]{3})\s*\((\d+)\)/is", $symbol, $regs)) {
				$symbol = $regs[1];
				$amount = $regs[2];
			}else
				$amount = 1;

			//Skipping second cell
			$rstart = $data['end'];
			$data = findData($rcnt, '<td>', '</td>', $rstart, true);
			...
			$rstart = $data['end'];
			$data = findData($rcnt, '<td>', '</td>', $rstart, true);
			$buyCash = $this->CheckRate(str_replace('-', '.', trim(strip_tags($data['data']))), $amount);

			//Looking for selling rate (fourth HTML table cell)
			$rstart = $data['end'];
			$data = findData($rcnt, '<td>', '</td>', $rstart, true);
			$sellCash = $this->CheckRate(str_replace('-', '.', trim(strip_tags($data['data']))), $amount);

...
class IturupBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.iturupbank.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$cnt = preg_replace("/<style[^>]+>/is", '<style>', $cnt);
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			$qty = $symbol == 'JPY' ? 100:1;

			//Passing by central bank rates
			$rstart = $data['end'];
			$data = findData($rcnt, '<td>','</td>', $rstart, true);

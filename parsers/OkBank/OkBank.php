...
class OkBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.okbank.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<div class="underblock">','<td colspan="3">', 0, true);
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			if(preg_match("/(До|От) (\d+)/", $symbol, $regs)) {
				if($regs[1] == 'От')
					$qty = $regs[2]/1;
				continue;
			}
		...
		foreach ($rates as $symbol => $qrates) {
			foreach ($qrates as $qty => $rate) {
				//Saving rates. There is no Non-cash rates on page. Passing zeros for Non-Cash values
				$this->AddRate(

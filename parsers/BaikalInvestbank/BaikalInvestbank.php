...
class BaikalInvestbank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.baikalinvestbank.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			if(preg_match("/за (\d+)/i", $rcnt, $regs)) {
				$qty = $regs[1];
			}

			//Looking for currency symbol (first HTML table cell)
			$rstart = 0;
			$data = findData($rcnt, '<th>', '</a>', $rstart, true);

...
class DiamantBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://diamantbank.ua/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<div class="currency">','<form id="currency_calculator_form">', 0, true);
		$cnt = trim($data['data']);

		// Unifying and modifying tags for simplifying search
		$cnt = preg_replace("/<div[^>]+>/is", '<div>', $cnt);
		$cnt = preg_replace("/<\/?(a href|img)[^>]*>/is", '', $cnt);

		//Looping trough all rates rows
		$start = 0;

		while(is_int($start) && $row = findData($cnt, '<div>', '</a>', $start, true)) {
			$rcnt = $row['data'];
			$start = $row['end'];

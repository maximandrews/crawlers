...
class BankPerm extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.bankperm.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<table class="curs">','</table>', 0, true);
		$cnt = trim($data['data']);
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			$rstart = $data['end']+5;
			$data = findData($rcnt, '<b class="">', '</b>', $rstart, true);

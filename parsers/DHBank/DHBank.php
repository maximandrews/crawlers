...
class DHBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.dhb.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, 'class="fort">','</table>', 0, true);
		$cnt = iconv('windows-1251', 'UTF-8', trim($data['data']));
		
		// Modifying tags for simplifying search
		$cnt = str_replace ('</tr>','<\tr>',$cnt);
		$cnt = str_replace ('</td>','<\td>',$cnt);
		$cnt = str_replace ('/','<\td><td>',$cnt);

		//Looping trough all rates rows
		$start = 0;
		$block = 0; // Row numbers with data
		while(is_int($start) && $row = findData($cnt, '<tr>', '<\tr>', $start, true)) {
			$rcnt = $row['data'];
			$start = $row['end'];
			$rstart = 0;
			if($block == 1) {
				for ($z = 1; $z <= 2; $z++) {
					...
					$data = findData($rcnt, '<td>', '<\td>', $rstart, true);

...
class DcapitalBank extends baseParser {
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, 'class="inn2">','</table>', 0, true);
		$cnt = iconv('windows-1251', 'UTF-8', trim($data['data']));

		//Unifying tags and removing extra tabs and spacing
		$cnt = preg_replace("/<td[^>]+>/is", '<td>', $cnt);
		$cnt = preg_replace("/<b[^>]+>/is", '<b>', $cnt);
		$cnt = preg_replace("/\s+/", "",$cnt);
		
		// Removing unnecessary symbols, headings and empty cells
		$cnt = str_replace('руб.','',$cnt);
		$cnt = str_replace('<td><b>Покупка</b>','',$cnt);
		$cnt = str_replace('><td><b>Продажа</b>','',$cnt);
		$cnt = str_replace('</tr><tr<b></td><td><b>','',$cnt);

...
class HimeksimBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.chemexim.ru/index.htm');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<div class="table_currency">','</table>', 0, true);
		$cnt = trim($data['data']);
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			//Looking for central bank rate, which won't be saved into variable
			$rstart = $data['end'];
			$data = findData($rcnt, '>', '</td>', $rstart, true);

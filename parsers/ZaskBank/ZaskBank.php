...
class ZaskBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.zaskb.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<table class="moduletable-new">','</table>', 0, true);
		$cnt = trim($data['data']);
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			// Setting quantity for minimal deal amount for each foreign currency
			$qty = preg_replace("/\D+/", '', $symbol);
			if($qty < 1) $qty = 1;

			$symbol = preg_replace("/[^A-Z]+/i", '', $symbol);

...
class BOC extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.boc.ru/index.php?lang=rus&n_id=12');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<table width="400">', '</table>', 0, true);
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			if(preg_match('/(\d+)\s*(\w{3})/is', $symbol, $regs)) {
				$amount = $regs[1];
				$symbol = $regs[2];
			}

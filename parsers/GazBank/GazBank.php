...
class GazBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.gazbank.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<div id="currency">','</table>', 0, true);
		$cnt = trim($data['data']);

		//Simplifying parsing
		$cnt = preg_replace("/<tr[^>]+>/i", '<tr>', $cnt);
		$cnt = preg_replace("/<td[^>]+>/i", '<td>', $cnt);

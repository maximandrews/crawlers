...
class Bankb extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.ankb.ru/components/com_kurs/ankb.php');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '</td> </tr>','</tbody>', 0, true);
		$cnt = trim($data['data']);
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			$rstart = $data['end'];
			$data = findData($rcnt, 'right>', '</', $rstart, true);

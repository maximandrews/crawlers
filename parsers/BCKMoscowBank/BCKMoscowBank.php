...
class BCKMoscowBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.bcc-msk.ru/bureaudechange');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<h1 class="title" id="page-title">','</tbody>', 0, true);
		$cnt = trim($data['data']);
						
		//Looping trough all rates rows
		$start = strpos($cnt, '</b></td></tr><tr>')-4;
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			//Replacing RUB with empty string
			$symbol = str_replace('RUB', '', $symbol);

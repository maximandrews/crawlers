...
class FidocomBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.fidocombank.com.ua/ua/private/currencies');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, '<table class="overview glaze">','</table>', 0, true);
		$cnt = trim($data['data']);
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			// Passing by currency names in Ukrainian 
			$rstart = $data['end'];
			$data = findData($rcnt, '<td>', '</td>', $rstart, true);

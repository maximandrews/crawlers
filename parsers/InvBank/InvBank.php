...
class InvBank extends baseParser {
	public function Init() {
		...
		*/
		$this->fetchUrl('Rates', 'http://www.invb.ru/');
	}
	...
	public function ParseRates($mdata=Array()) {
		...
		*/
		...
		$data = findData($data, ' <table width="100%" border="0" cellspacing="0" cellpadding="0" class="valuta">', '</table>', 0, true);
		$cnt = iconv('koi8-r', 'UTF-8', trim($data['data']));
		...
		while(is_int($start) && $row = findData($cnt, '<tr>', '</tr>', $start, true)) {
			...
			$symbol = trim(findData($data['data'], 'all/', '.', 0, true)['data']);
